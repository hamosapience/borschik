 ...
 * @param {Boolean} webp If true convert image to webp format.
 * @returns {String} New path of processed image.
 */
exports.processPath = function(filePath, webp) {
    return freeze(realpathSync(filePath), undefined, webp);
};
 ...
 */
var freeze = exports.freeze = function(filePath, content, webp) {
    ...
    if (_freezeDir) {
        ...
        if (_freezeDir === ':base64:' || _freezeDir === ':encodeURIComponent:' || _freezeDir === ':encodeURI:') {
            ...
            var excluded = !configWebpEnabled(_freezeDir);
            if (webp && config.webp.excludePaths.length) {
                config.webp.excludePaths.map(function(item){
                    // console.log(item, _freezeDir, _freezeDir.match(item));
                    if (filePath.match(item)){
                        excluded = true;
                    }
                });
            }
            ...
            if ( webp && !excluded ) {
                filePathWebp = PATH.join(_freezeDir, nestingPath + ".webp");
                if (content && !FS.existsSync(filePathWebp)) {
                    convertToWebp(filePath, filePathWebp);
                }
                filePath = filePathWebp;
            }
            else {
...
/**
 * Returns if webp enabled for given path.
 * @param {String} path Path to search config for.
 * @returns {Number}
 */
var configWebpEnabled = function(path) {
    var webp = config.webp;

    return webp && (!webp.paths || config.webp.paths[path] || false);
};
...
var loadConfig = exports.loadConfig = function(path) {
    ...
    if (FS.existsSync(config_path)) {
        ...
        for (var freezeConfigWildcard in freezePaths) {
            ...
            if (!config.freezeWildcards[freezeRealPath]) {
        ...
        var webp = _config.webp,
            webp_paths = _config.webp_paths;
        if (webp) {
            if (webp_paths) {
                config.webp.paths = {};
                webp_paths.forEach(function(dir) {
                    var realpath = realpathSync(PATH.resolve(path, dir));
                    config.webp.paths[realpath] = true;
                });
            }
    
            config.webp.lossless = _config.webp.lossless || false;
            config.webp.quality = _config.webp.quality || "90";
            config.webp.excludePaths = [];
            if (_config.webp.exclude_paths && _config.webp.exclude_paths.length) {
                for (var i = 0; i < _config.webp.exclude_paths.length; i++) {
                    config.webp.excludePaths.push(new RegExp(_config.webp.exclude_paths[i]));
                }
            }
        }
    } else {
 ...
 *
 * @param {String} inputFilePath Path to original file
 * @param {String} outputFilePath File path to save.
 * @param {Function} callback Optional callback invoked after convert
 */
 var convertToWebp = exports.convertToWebp = function (inputFilePath, outputFilePath, callback){
    ...
    mkpath(PATH.dirname(outputFilePath));
    var params = [];
    params.push(" " + inputFilePath);
    params.push("-o "+ outputFilePath);
    config.webp.lossless && params.push("-lossless");
    config.webp.quality && !config.webp.lossless && params.push("-q " + config.webp.quality);
    try{
        var out = EXEC(CWEBP + params.join(" "));
        console.log('BORSCHIK WEBP says:', out.toString());
    } catch(e) {
        console.log('BORSCHIK WEBP failed', e && e.stderr && e.stderr.toString() || e);
    }
};
...
//Convertable to webp images extenstions
var convertableImgExt = (process.env.BORSCHIK_CONVERTABLE_IMG_EXTS ||
        'jpg jpeg png').split(' '),
    convertableImgRe = new RegExp('\\.(' + convertableImgExt.join('|') + ')$');
...
/** 
 * Check if URL is an convertible to webp format image.
 * 
 * @param {String} url URL to check.
 * @return {boolean} True if file is convertible image, otherwise false.
 */
exports.isConvertibleImage = function(url) {
    return convertableImgRe.test(url);
};
