...
var loadConfig = exports.loadConfig = function(path) {
    ...
    if (FS.existsSync(config_path)) {
        ...
        for (var dir in freezeNestingLevels) {
            var realpath = realpathSync(PATH.resolve(path, dir));
            if (!config.freezeNestingLevel[realpath]) {
                var value = Math.max(parseInt(freezeNestingLevels[dir], 10) || 0, 0);
                config.freezeNestingLevel[realpath] = value;
            }
        }
        ...
        for (var freezeConfigWildcard in freezePaths) {
            ...
            if (!config.freezeWildcards[realpath]) {
                ...
                if (freezeToPath !== ':base64:' && freezeToPath !== ':encodeURIComponent:' && freezeToPath !== ':encodeURI:') {
                    ...
                    /* может быть осмысленное значение - 0 */
                    if (config.freezeNestingLevel[freezeToPath] === undefined) {
...
var freezableExts = (process.env.BORSCHIK_FREEZABLE_EXTS ||
        'jpg jpeg gif ico png swf svg ttf eot otf woff webp').split(' '),
